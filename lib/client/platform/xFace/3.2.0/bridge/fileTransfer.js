...
//       Currently, use what emulator/fs uses (TEMP).
var _self,
    fs = ripple('fs'),
    FILENAME = /\/?([^\/]*)$/,
...
//       They are needed because this module uses emulator/fs vs the stock HTML FileSystem API (with good reason..).
    bbFSPaths = {
        sandboxed: "/emulatorFileSystem/" + _appId + "/sandbox",
        unSandboxed: "/emulatorFileSystem/" + _appId + "/unsandboxed"
    };
...
_self = {
    download: function (success, error, args) {
        console.log('FileTransfer download is invoked:' + args[0]);
        download(args[0], args[1], success, error);
    },
    upload: function (success, error, args) {
        console.log('FileTransfer upload is invoked!' + args[0]);
        ...
        upload(args[0], args[1], success, error, opts);
    },
    abort: function (success, error, args) {
        console.log('FileTransfer abort is invoked!');
    }
};
