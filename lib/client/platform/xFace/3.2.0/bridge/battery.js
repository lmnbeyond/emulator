...
module.exports = {
    start: function (success, error, args) {
        event.on("DeviceBatteryStateChanged", function (charging) {
             var info = {
                 isPlugged: charging,
                 level: parseInt(deviceSettings.retrieve('battery.level'), 10)
             }
             return success && success(info);
        });
        event.on("DeviceBatteryLevelChanged", function (level) {
             var info = {
                 isPlugged: deviceSettings.retrieve('battery.state'),
    ...
    stop: function (success, error, args) {
        event.clear("DeviceBatteryStateChanged");
        event.clear("DeviceBatteryLevelChanged");
    }
};
